...
for folder in ['maps', 'control', 'dmm']:
...
for k in trange(NUM_REPLAYS):
    while (j <= k):
        ...
        try:
            ...
            # Deep motion masking replays
            noise = np.random.normal(size=32).astype('float16')
            frames = dmm(np.repeat([frames], 500, axis=0), np.repeat([noise], 500, axis=0))[0]
            xror.data['frames'] = np.hstack([times, frames])
            bsor = xror.toBSOR()
            with open('./interactivity/dmm/' + replay, 'wb') as f:
                f.write(bsor)
