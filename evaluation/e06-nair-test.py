...
dmmFeatures = np.load('./anonymity/nair-dmm.npy')
dmmTrainX = dmmFeatures[:,:300].reshape(-1, 211)
dmmTestX = dmmFeatures[:,300:].reshape(-1, 211)
...
# Train adaptive nair model (DMM)
print("Training Nair model (DMM)...")
model = train(dmmTrainX, trainY)
test(model, controlTestX, "Control")
test(model, metaguardTestX, "MetaGuard")
test(model, dmmTestX, "DMM")
