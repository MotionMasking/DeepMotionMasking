...
dmmFeatures = np.load('./anonymity/miller-dmm.npy')
dmmTrainX = dmmFeatures[:,:300].reshape(-1, 105)
dmmTestX = dmmFeatures[:,300:].reshape(-1, 105)
...
# Train adaptive miller model (DMM)
print("Training Miller model (DMM)...")
rf = trainrf(dmmTrainX, trainY)
testrf(rf, controlTestX, "Control")
testrf(rf, metaguardTestX, "MetaGuard")
testrf(rf, dmmTestX, "DMM")
