...
dmmFeatures = np.load('./anonymity/funnel-dmm.npy')
dmmTrainX = dmmFeatures[:,:10].reshape(-1, 900, 21)
dmmTestX = dmmFeatures[:,10:].reshape(-1, 900, 21)
...
# Train adaptive funnel model (DMM)
print("Training LSTM model (DMM)...")
model = train(dmmTrainX, train_labels)
test(model, controlTestX, "Control")
test(model, metaguardTestX, "MetaGuard")
test(model, dmmTestX, "DMM")
